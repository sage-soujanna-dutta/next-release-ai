...
export class ConfluenceService {
  ...
  async publishPage(htmlContent: string, sprintNumber?: string): Promise<string> {
    try {
      ...
      if (existingPages.length > 0) {
        ...
        const response = await axios.post(
          `https://${this.host}/wiki/rest/api/content`,
          payload,
  ...
  async updatePage(pageId: string, htmlContent: string, sprintNumber?: string): Promise<string> {
    try {
      // First get the current page to get the version
      const pageResponse = await axios.get(
        `https://${this.host}/wiki/rest/api/content/${pageId}`,
        { headers: this.getAuthHeaders() }
      );
  ...
  async searchPages(query: string): Promise<any[]> {
    try {
      ...
      const response = await axios.get(
        `https://${this.host}/wiki/rest/api/content/search?cql=space="${this.spaceKey}" AND title="${query}"`,
        { headers: this.getAuthHeaders() }
      );
