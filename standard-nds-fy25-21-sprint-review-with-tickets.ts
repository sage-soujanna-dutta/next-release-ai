...
/**
 * STANDARD SPRINT REVIEW REPORT - NDS-FY25-21 WITH JIRA TICKETS LIST
 ...
 * 
 * USAGE:
 * npx tsx standard-nds-fy25-21-sprint-review-with-tickets.ts
 */
...
// ===================================================================
async function fetchNDSSprintWithTickets(boardId: number, sprintNumber: string): Promise<StandardSprintReviewData> {
    ...
    try {
        console.log(`🔍 Fetching NDS sprint review data for ${sprintNumber}...`);
        ...
        let targetSprint = sprintsRes.data.values.find((sprint: SprintJiraData) => 
            sprint.name === `NDS-${sprintNumber}` || 
            sprint.name.includes(sprintNumber) || 
            sprint.name.includes('FY25-21')
        );
...
// ===================================================================
function generateSprintReviewWithTickets(data: StandardSprintReviewData): any {
...
- **📝 Plan remaining work** for next sprint
- **🤝 Gather feedback** from product owners
- **📊 Update velocity** and capacity planning
        ...
        "potentialAction": [
            ...
            },
            {
                ...
                "targets": [
                    {
                        "os": "default",
                        "uri": `https://${process.env.JIRA_DOMAIN}/secure/RapidBoard.jspa?rapidView=${SPRINT_CONFIG.boardId}&view=planning.nodetail`
                    }
                ]
            }
        ]
...
async function sendToTeams(messageCard: any): Promise<void> {
    ...
    try {
        console.log('📤 Sending sprint review report to Teams...');
...
// MAIN EXECUTION
// ===================================================================
async function generateNDSSprintReviewWithTickets() {
    console.log('📋 STANDARD SPRINT REVIEW REPORT WITH TICKETS');
    console.log('==============================================');
    ...
    try {
        ...
        const sprintData = await fetchNDSSprintWithTickets(SPRINT_CONFIG.boardId, SPRINT_CONFIG.sprintNumber);
        
        // Generate Teams notification with tickets
        const teamsNotification = generateSprintReviewWithTickets(sprintData);
        
        // Send to Teams
        await sendToTeams(teamsNotification);
        
        console.log('\n✅ STANDARD NDS-FY25-21 Sprint Review with Tickets completed!');
...
generateNDSSprintReviewWithTickets();
